...
function buildClient(name, actions, fetcher) {
  return `
'use client';
...
import { create${name}, update${name}, delete${name} } from '../../actions/${actions}';
...
export default function Page() {
  return (
    <div className="space-y-6 md:space-y-8 h-full animate-fade-in pb-20">
      <PageHeader title="${name}" description="Quản lý dữ liệu" />
...
const modules = [
  { name: 'Journal', route: 'journals', actions: 'journals', fetcher: 'fetchJournals' },
  { name: 'Reconciliation', route: 'reconciliation', actions: 'reconciliation', fetcher: 'fetchReconciliation' }
];

modules.forEach(m => {
  const p = path.join(root, m.route);
  if (!fs.existsSync(p)) fs.mkdirSync(p, { recursive: true });
  fs.writeFileSync(path.join(p, m.name + 'Client.tsx'), buildClient(m.name, m.actions, m.fetcher));
