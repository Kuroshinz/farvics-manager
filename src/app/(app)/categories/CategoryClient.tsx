...
import { createCategory, updateCategory, deleteCategory, archiveCategory, restoreCategory } from '../../actions/categories';
...
export function CategoryClient({ initialData }: { initialData: any[] }) {
  ...
  const onSubmit = async (vals: any) => {
    const res = editingItem ? await updateCategory(editingItem.id, vals) : await createCategory(vals);
    if (res.ok) {
  ...
  const columns = [
    { key: 'name', header: 'Tên danh mục', render: (i: any) => i.name },
    ...
    { key: 'actions', header: '', render: (i: any) => <ActionMenu onEdit={() => { setEditingItem(i); setDrawerOpen(true); }} onDuplicate={() => toast.info('Đã nhân bản')} onArchive={() => archiveCategory(i.id).then(() => { toast.success('Đã lưu trữ'); router.refresh(); })} onDelete={() => setDeleteItem(i)} /> }
  ];
    ...
    <div className="h-[600px] relative">
      ...
      <FormDrawer open={isDrawerOpen} onOpenChange={(o) => { setDrawerOpen(o); if (!o) setEditingItem(null); }} title={editingItem ? 'Chỉnh sửa' : 'Tạo mới'} description="Nhập thông tin chi tiết.">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div><label className="block text-xs font-medium text-content-secondary mb-1">Tên danh mục</label><input  {...register('name')} className="w-full bg-white/5 border border-white/10 rounded-lg p-2.5 text-white outline-none" />{errors.name && <span className="text-galaxy-red text-xs">{errors.name.message as string}</span>}</div>
