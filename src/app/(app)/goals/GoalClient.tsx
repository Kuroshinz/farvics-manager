...
import { createGoal, updateGoal, deleteGoal, archiveGoal, restoreGoal } from '../../actions/goals';
import { Typography } from '../../../components/ui/typography/Typography';
import { useRouter } from 'next/navigation';

const schema = z.object({
  name: z.string().min(1, 'Bắt buộc'), target_amount: z.number().min(0, '>= 0')
});

export function GoalClient({ initialData }: { initialData: any[] }) {
  ...
  const onSubmit = async (vals: any) => {
    const res = editingItem ? await updateGoal(editingItem.id, vals) : await createGoal(vals);
    if (res.ok) {
  ...
  const handleConfirmDelete = async () => {
    ...
    const res = await deleteGoal(deleteItem.id);
    if (res.ok) {
  ...
  const columns = [
    { key: 'name', header: 'Tên mục tiêu', render: (i: any) => i.name },
    { key: 'target_amount', header: 'Mục tiêu', render: (i: any) => formatCurrency(Number(i.target_amount||0)/100) },
    ...
    { key: 'actions', header: '', render: (i: any) => <ActionMenu onEdit={() => { setEditingItem(i); setDrawerOpen(true); }} onDuplicate={() => toast.info('Đã nhân bản')} onArchive={() => archiveGoal(i.id).then(() => { toast.success('Đã lưu trữ'); router.refresh(); })} onDelete={() => setDeleteItem(i)} /> }
  ];
    ...
    <div className="h-[600px] relative">
      ...
      <FormDrawer open={isDrawerOpen} onOpenChange={(o) => { setDrawerOpen(o); if (!o) setEditingItem(null); }} title={editingItem ? 'Chỉnh sửa' : 'Tạo mới'} description="Nhập thông tin chi tiết.">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div><label className="block text-xs font-medium text-content-secondary mb-1">Tên mục tiêu</label><input  {...register('name')} className="w-full bg-white/5 border border-white/10 rounded-lg p-2.5 text-white outline-none" />{errors.name && <span className="text-galaxy-red text-xs">{errors.name.message as string}</span>}</div>
          <div><label className="block text-xs font-medium text-content-secondary mb-1">Mục tiêu</label><input type="number" {...register('target_amount', { valueAsNumber: true })} className="w-full bg-white/5 border border-white/10 rounded-lg p-2.5 text-white outline-none" />{errors.target_amount && <span className="text-galaxy-red text-xs">{errors.target_amount.message as string}</span>}</div>
