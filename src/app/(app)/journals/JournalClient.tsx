...
import { createJournal, updateJournal, deleteJournal } from '../../actions/journals';
import { Typography } from '../../../components/ui/typography/Typography';
import { useRouter } from 'next/navigation';

const schema = z.object({ id: z.string().optional() });

export function JournalClient({ initialData }: { initialData: any[] }) {
  ...
  const onSubmit = async (vals: any) => {
    const res = editingItem ? await updateJournal(editingItem.id, vals) : await createJournal(vals);
    if (res.ok) {
  ...
  const handleConfirmDelete = async () => {
    ...
    const res = await deleteJournal(deleteItem.id);
    if (res.ok) {
