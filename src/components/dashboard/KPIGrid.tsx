...
export function KPIGrid({ data = { revenue: 2160000, saving: 1194000, expense: 1663000, net: 910000 } }: { data?: any }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
      <KPICard title="Total Revenue" value={`${(data.revenue).toLocaleString()} đ`} change="12.6 %" isPositive={true} icon={<DollarSign size={16} />} isPrimary={true} delay={0.1} />
      <KPICard title="Total Saving" value={`${(data.saving).toLocaleString()} đ`} change="2.4 %" isPositive={true} icon={<PiggyBank size={16} />} delay={0.2} />
      <KPICard title="Total Expense" value={`${(data.expense).toLocaleString()} đ`} change="4.1 %" isPositive={false} icon={<CreditCard size={16} />} delay={0.3} />
      <KPICard title="Net Profit" value={`${(data.net).toLocaleString()} đ`} change="8.2 %" isPositive={true} icon={<Sparkles size={16} />} delay={0.4} />
    </div>
  );
}
